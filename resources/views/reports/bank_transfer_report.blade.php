...
<head>
    <title>Bank Transfer Register</title>
    <style>
        @page {
    ...
    <header>
        ...
        <div class="status-bar clearfix">
            <div class="status-left">
                REPORT TYPE: {{ strtoupper($reportTitle ?? 'BANK DETAILS REPORT') }} | PERIOD: {{ $period ?? now()->format('d-M-Y') }}
            </div>
        </div>
    </header>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width:22%;">BANK / BRANCH</th>
                ...
                @forelse($reportData as $row)
                    <tr class="{{ $sn % 2 == 0 ? 'row-alt' : '' }}">
                        ...
                        <td class="text-left">
                            ...
                            <div style="color:#334155; font-size:7px; font-weight:600; text-transform:uppercase;">
                                {{ $row['name'] }}
                            </div>
                        </td>
                        <td class="text-left" style="font-size:7px;">{{ $row['account_holder_name'] }}</td>
                        <td class="text-left" style="font-size:7px;">
                            <div style="font-weight:600; color:#1e3a5f;">{{ $row['bank_name'] }}</div>
                            <div style="color:#64748b; font-size:6.5px;">{{ $row['bank_branch'] }}</div>
                        </td>
                        <td class="text-center">
                            <span class="badge-account">{{ $row['account_number'] }}</span>
                        </td>
                        <td class="text-center">
                            <span class="badge-ifsc">{{ $row['ifsc_code'] }}</span>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="text-center" style="padding:20px; color:#64748b; font-style:italic;">
                            No bank details found for the selected filters.
                        </td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr class="total-row">
                    ...
                    <td class="text-center">{{ $reportData->count() }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
