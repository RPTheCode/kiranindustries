...
<html>
<head>
    <title>Nominee Declaration Register</title>
    <style>
        @page {
    ...
    <header>
        ...
        <div class="status-bar clearfix">
            <div class="status-left">
                REPORT TYPE: {{ strtoupper($reportTitle ?? 'NOMINEE DECLARATION REGISTER') }} | PERIOD: {{ $period ?? now()->format('d-M-Y') }}
            </div>
        </div>
    </header>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width:22%;">NOMINEE NAME</th>
                    <th style="width:16%;">RELATION</th>
                    <th style="width:12%;">SHARE %</th>
                    <th style="width:22%;">AADHAAR NO.</th>
                </tr>
            </thead>
            <tbody>
                @php 
                    $sn = 1; 
                    $grouped = collect($reportData)->groupBy('code');
                @endphp
                @forelse($grouped as $empCode => $nominees)
                    @foreach($nominees as $index => $row)
                    <tr class="{{ $sn % 2 == 0 ? 'row-alt' : '' }}">
                        @if($index === 0)
                            <td class="text-center font-bold" rowspan="{{ count($nominees) }}">{{ $sn++ }}</td>
                            <td class="text-left" rowspan="{{ count($nominees) }}" style="border-right: 1px solid #e2e8f0; vertical-align: middle;">
                        ...
                        @endif
                        <td class="text-left" style="font-weight:600; color:#1e293b;">{{ $row['nominee_name'] }}</td>
                        <td class="text-center">
                            <span class="badge-relation">{{ $row['relation'] }}</span>
                        </td>
                        <td class="text-center">
                            <span class="badge-share">{{ $row['share'] }}</span>
                        </td>
                        <td class="text-center" style="font-size:7px; color:#475569; letter-spacing:0.5px;">
                            {{ $row['aadhar'] ?: '—' }}
                        </td>
                    </tr>
                    @endforeach
            ...
            <tfoot>
                <tr class="total-row">
                    <td colspan="5" class="text-right" style="padding-right:8px;">TOTAL EMPLOYEES</td>
                    <td class="text-center">{{ isset($grouped) ? $grouped->count() : 0 }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
