...
<head>
    ...
    <style>
        ...
        .deduction-section { background-color: #fef2f2; color: #991b1b; }
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="earning-section" style="width: 32%;">EARNINGS</th>
                    <th class="deduction-section" style="width: 25%;">DEDUCTIONS</th>
            ...
            <tbody>
                @php 
                    ...
                    foreach($deductionColumns as $col) { $colTotals['deductions'][$col] = 0; }
                @endphp
                ...
                @foreach($reportData as $deptName => $rows)
                    ...
                    @foreach($rows as $row)
                        ...
                        <tr class="{{ $sr % 2 == 0 ? 'row-alt' : '' }}">
                            ...
                            <!-- Dynamic Deductions Column -->
                            <td class="text-left">
                                @php 
                                    $hasDeductions = false;
                                    $activeDeductions = [];
                                    foreach($deductionColumns as $col) {
                                        $info = $row['deductions'][$col] ?? ['amount' => 0, 'mode' => 'amount', 'qty' => 0];
                                        $val = (float)($info['amount'] ?? 0);
                                        $colTotals['deductions'][$col] += $val;
                                        if ($val > 0) {
                                            $activeDeductions[$col] = $info;
                                            $hasDeductions = true;
                                        }
                                    }
                                @endphp
                                @if($hasDeductions)
                                    <table class="particulars-table">
                                        @foreach($activeDeductions as $col => $info)
                                            <tr>
                                                <td class="text-left font-bold" style="color: #991b1b; width: 45%;">{{ $col }}</td>
                                                <td class="text-center" style="width: 25%;">
                                                    @if($info['mode'] === 'day')
                                                        <span style="background-color: #f3e8ff; color: #6b21a8; border: 1px solid #e9d5ff; padding: 1px 3px; font-size: 5.5px; font-weight: bold; border-radius: 3px; display: inline-block; white-space: nowrap;">DAYS: {{ round($info['qty'], 1) }}</span>
                                                    @else
                                                ...
                                                <td class="text-right font-bold" style="color: #991b1b; width: 30%;">{{ number_format($info['amount'], 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </table>
                                @else
                            ...
                            <td class="text-left">
                                <div style="padding: 2px 0;">
                                    <table class="particulars-table">
                                        <tr>
                                            <td class="text-left font-bold" style="font-size: 7px; color: #166534; width: 40%;">GROSS</td>
                                            <td class="text-right font-bold" style="font-size: 7px; color: #166534; width: 60%;">Rs. {{ number_format($row['total_earning'], 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-left font-bold" style="font-size: 7px; color: #991b1b; width: 40%;">DED.</td>
                                            <td class="text-right font-bold" style="font-size: 7px; color: #991b1b; width: 60%;">Rs. {{ number_format($row['total_deduction'], 2) }}</td>
                                        </tr>
                                        <tr style="border-top: 1px solid #cbd5e0;">
                                            <td class="text-left font-bold" style="font-size: 7.5px; color: #0369a1; padding-top: 3px !important; width: 40%;">NET</td>
                                            <td class="text-right font-bold" style="font-size: 7.5px; color: #0369a1; padding-top: 3px !important; width: 60%;">Rs. {{ number_format($row['net_pay'], 2) }}</td>
                                        </tr>
                                    </table>
                                </div>
                            </td>
            ...
            <tfoot>
                <tr class="total-row" style="background-color: #f1f5f9;">
                    ...
                    <!-- Dynamic Deductions Grand Totals -->
                    <td class="text-left">
                        @php 
                            $hasGrandDeductions = false;
                            $activeGrandDeductions = [];
                            foreach($deductionColumns as $col) {
                                $sum = $colTotals['deductions'][$col] ?? 0;
                                if ($sum > 0) {
                                    $activeGrandDeductions[$col] = $sum;
                                    $hasGrandDeductions = true;
                                }
                            }
                        @endphp
                        @if($hasGrandDeductions)
                            <table class="particulars-table">
                                @foreach($activeGrandDeductions as $col => $sum)
                                    <tr>
                                        <td class="text-left font-bold" style="color: #991b1b; width: 50%;">{{ $col }}</td>
                                        <td class="text-center" style="width: 20%;">&nbsp;</td>
                                        <td class="text-right font-bold" style="color: #991b1b; width: 30%;">{{ number_format($sum, 2) }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        @else
                    ...
                    <td class="text-left">
                        <div style="padding: 2px 0;">
                            <table class="particulars-table">
                                <tr>
                                    <td class="text-left font-bold" style="font-size: 7px; color: #166534; width: 40%;">GROSS</td>
                                    <td class="text-right font-bold" style="font-size: 7px; color: #166534; width: 60%;">Rs. {{ number_format($colTotals['total_earning'], 2) }}</td>
                                </tr>
                                <tr>
                                    <td class="text-left font-bold" style="font-size: 7px; color: #991b1b; width: 40%;">DED.</td>
                                    <td class="text-right font-bold" style="font-size: 7px; color: #991b1b; width: 60%;">Rs. {{ number_format($colTotals['total_deduction'], 2) }}</td>
                                </tr>
                                <tr style="border-top: 1px solid #cbd5e0;">
                                    <td class="text-left font-bold" style="font-size: 7.5px; color: #0369a1; padding-top: 3px !important; width: 40%;">NET</td>
                                    <td class="text-right font-bold" style="font-size: 7.5px; color: #0369a1; padding-top: 3px !important; width: 60%;">Rs. {{ number_format($colTotals['net_pay'], 2) }}</td>
                                </tr>
                            </table>
                        </div>
                    </td>
        ...
        if (isset($pdf)) {
            ...
            $pdf->page_script('
                if ($PAGE_COUNT > 1 && $PAGE_NUM < $PAGE_COUNT) {
                    $font = $fontMetrics->get_font("helvetica", "bold");
                    $pdf->text(400, 560, "Continued on Page No... " . ($PAGE_NUM + 1), $font, 8, array(0.2, 0.2, 0.2));
                }
            ');
         }
    </script>
