...
export function CrudFormModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      formConfig.fields.forEach(field => {
        ...
        // Special handling for date fields to ensure they are in YYYY-MM-DD format
        // and match local timezone immediately on load.
        if (field.type === 'date' && cleanData[field.name]) {
          if (window.appSettings?.formatDateForInput) {
            cleanData[field.name] = window.appSettings.formatDateForInput(cleanData[field.name]);
          }
        }
  ...
  const handleChange = (name: string, value: any) => {
    setFormData(prev => {
      ...
      if (field?.onChange) {
        // Use a local setter to modify 'next' directly within this atomic update
        const sideEffectSetter = (setter: any) => {
        ...
        field.onChange(value, next, sideEffectSetter);
      }
    ...
    formConfig.fields.forEach(field => {
      ...
      if (field.validation?.pattern && formData[field.name]) {
        ...
        if (!regex.test(formData[field.name])) {
          newErrors[field.name] = `${field.label} ${t('format is invalid')}`;
        }
      }
      ...
      if (field.type === 'email' && formData[field.name]) {
        ...
        if (!emailRegex.test(formData[field.name])) {
          newErrors[field.name] = t('Please enter a valid email address');
        }
      }
    });
  ...
  const renderField = (field: FormField) => {
    ...
    // Safely resolve the options array, handling both direct arrays and potential callback functions
    ...
    if (mode === 'view') {
      ...
      if (field.type === 'multi-select') {
        ...
        const selectedLabels = selectedValues
          .map((value: string) => {
            const option = (resolvedOptions as any[]).find(opt => String(opt.value) === String(value));
      ...
      return (
        <div className="p-2 border rounded-md bg-gray-50">
          {field.type === 'select' || field.type === 'combobox' ?
            (resolvedOptions as any[]).find(opt => String(opt.value) === String(formData[field.name]))?.label || formData[field.name] || '-' :
            field.type === 'checkbox' ?
    ...
    switch (field.type) {
      case 'combobox':
        const comboOptions = field.relation
          ? relationOptions[field.name] || []
          : (typeof field.options === 'function' ? field.options(formData) : field.options || []);

        const formattedOptions = field.relation
          ? comboOptions.map((opt: any) => ({
            label: opt[field.relation!.labelField],
            value: String(opt[field.relation!.valueField])
          }))
          : comboOptions.map((opt: any) => ({
            label: opt.label,
          ...
          <Input
            ...
            onChange={(e) => {
              if (field.validation?.pattern) {
                // If it's a numeric pattern, restrict input immediately
      ...
      case 'password':
        const isPasswordVisible = showPasswords[field.name];
        return (
      ...
      case 'date':
        ...
        let dateValue = '';
        const rawDate = formData[field.name];

        if (rawDate) {
          if (window.appSettings?.formatDateForInput) {
            dateValue = window.appSettings.formatDateForInput(rawDate);
          } else {
            // Fallback for cases where appSettings might not be initialized
        ...
        return (
          <Input
            ...
            onChange={(e) => {
              ...
              if (!isNaN(numValue)) {
                if (field.min !== undefined && numValue < (field.min as number)) {
                  numValue = field.min as number;
                }
                if (field.max !== undefined && numValue > (field.max as number)) {
                  numValue = field.max as number;
                }
      ...
      case 'select':
        const selectOptions = field.relation
          ? relationOptions[field.name] || []
          : (typeof field.options === 'function' ? field.options(formData) : field.options || []);

        const currentSelectValue = formData[field.name] !== undefined && formData[field.name] !== null 
          ? String(formData[field.name]) 
          : '';

        const comboboxOptions = selectOptions.map((opt: any) => ({
          label: field.relation ? String(opt[field.relation.labelField]) : String(opt.label),
          value: field.relation ? String(opt[field.relation.valueField]) : String(opt.value)
        }));
      ...
      case 'radio':
        const radioOptions = field.relation
          ? relationOptions[field.name] || []
          : (typeof field.options === 'function' ? field.options({ data: formData }) : field.options || []);

        const resolvedRadioOptions = radioOptions.map((opt: any) => ({
          label: field.relation ? String(opt[field.relation.labelField]) : String(opt.label),
          value: field.relation ? String(opt[field.relation.valueField]) : String(opt.value)
        }));
        ...
        return (
          ...
          >
            {resolvedRadioOptions.map((option: any) => (
      ...
      case 'multi-select':
        return (
          <MultiSelect
            options={resolvedOptions.map((opt: any) => ({ label: String(opt.label), value: String(opt.value) }))}
            selected={Array.isArray(formData[field.name]) ? formData[field.name] : []}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`${getModalSizeClass()} max-h-[90vh] flex flex-col overflow-hidden`} modalId={modalId}>
        <DialogHeader>
        ...
        <DialogFooter className="sm:justify-end pt-4 mt-auto border-t">
