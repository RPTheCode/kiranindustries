...
export function initializeGlobalSettings(settings: Record<string, any>) {
    ...
    window.appSettings = {
        ...
        formatDateForInput: (date: string | Date) => {
            if (!date) return '';
            try {
                // Use en-CA to force YYYY-MM-DD
                const timezone = settings.defaultTimezone || 'UTC';
                const d = typeof date === 'string' ? new Date(date) : date;

                // If it's a date-only string like "2025-12-20", new Date() might treat it as UTC.
                // We want to ensure it uses the timezone consistently.
        ...
        formatDuration: (hoursDecimal: number | string) => {
            if (hoursDecimal === null || hoursDecimal === '' || isNaN(Number(hoursDecimal))) return '-';
            const totalHours = Number(hoursDecimal);
