...
export default function Users() {
  ...
  if (hasPermission(permissions, 'create-users')) {
    ...
    pageActions.push({
      ...
      variant: canCreate ? 'default' : 'outline' as any,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: any, row: any) => {
        if (row.type === 'company' || row.type === 'admin') return <span className="text-muted-foreground">{t('All Branches')}</span>;
        if (!value || !value.length) return <span className="text-muted-foreground">{t('None')}</span>;

        return (
          <div className="flex flex-wrap gap-1">
            {value.map((branch: any) => (
              <span key={branch.id} className="inline-flex items-center rounded-md bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 ring-1 ring-inset ring-emerald-700/10">
                {branch.name}
              </span>
            ))}
          </div>
