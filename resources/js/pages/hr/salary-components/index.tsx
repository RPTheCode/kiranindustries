...
export default function SalaryComponents() {
  ...
  const { auth, salaryComponents, branches, filters: pageFilters = {} } = usePage().props as any;
    ...
    },
    {
      ...
      render: (value: string, row: any) => (
        <span className="text-gray-700 dark:text-gray-300">
          {row.branch?.name || t('All Branches')}
        </span>
      )
    },
    {
    ...
    },
    {
      ...
      render: (value: string) => (
        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${value === 'fixed'
          ...
          {value === 'fixed' ? t('Fixed') : (value === 'percentage_of_gross' ? t('Percentage of Gross') : t('Percentage'))}
        </span>
      )
    },
    {
      ...
      render: (value: any, row: any) => (
        <span className="font-mono">
          {row.calculation_type === 'fixed'
            ...
            : row.calculation_type === 'percentage_of_gross'
              ? `${row.percentage_of_gross_pay}%`
  ...
  const calculationTypeOptions = [
    ...
    { value: 'percentage_of_gross', label: t('Percentage of Gross') }
  ];
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              min: 0,
              step: 0.01,
              conditional: (mode: any, data: any) => data.calculation_type === 'fixed'
            },
            {
              name: 'percentage_of_basic',
              label: t('Percentage of Basic'),
              type: 'number',
              min: 0,
              max: 100,
              step: 0.01,
              conditional: (mode: any, data: any) => data.calculation_type === 'percentage'
            },
            {
              name: 'percentage_of_gross_pay',
              label: t('Percentage of Gross Pay'),
              type: 'number',
              min: 0,
              max: 100,
              step: 0.01,
              conditional: (mode: any, data: any) => data.calculation_type === 'percentage_of_gross'
            },
            /* {
                          ...
                          options: [
                            ...
                            { value: 'round', label: t('Round (Nearest)') },
                            { value: 'ceil', label: t('Ceil (Next Rupee)') },
                            { value: 'floor', label: t('Floor (Previous Rupee)') }
                          ],
                          defaultValue: 'none'
                        }, */
            // Checkbox to indicate whether this component should be treated as taxable in payroll.
            // { name: 'is_taxable', label: t('Is Taxable'), type: 'checkbox', defaultValue: true },
            // Checkbox to indicate whether this component is mandatory for the employee.
