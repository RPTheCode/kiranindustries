...
import { ArrowLeft, Download, Users, DollarSign, Calendar, FileText, RefreshCw } from 'lucide-react';
...
export default function PayrollRunShow() {
  ...
  const { payrollRun, heldCount = 0, heldAmount = 0 } = usePage().props as any;
  ...
  const canRegenerateEmployee = ['draft', 'processing', 'in_review', 'completed'].includes(payrollRun.status);

  const handleRegenerateEmployee = (entry: any) => {
    if (!entry?.employee_id) {
      return;
    }
    toast.loading(t('Recalculating employee payroll...'));
    router.post(
      route('hr.payroll-runs.regenerate-employee', payrollRun.id),
      { employee_id: entry.employee_id },
      {
        ...
        onError: () => {
          toast.dismiss();
          toast.error(t('Failed to regenerate employee payroll'));
        },
      }
    );
  };

  const handleEntryAction = (action: string, entry: any) => {
    if (action === 'regenerate-employee') {
      handleRegenerateEmployee(entry);
    } else if (action === 'generate-payslip') {
      toast.loading(t('Generating payslip...'));
      router.post(route('hr.payslips.generate'), {
        payroll_entry_ids: [entry.id]
      }, {
        onSuccess: (page: any) => {
          toast.dismiss();
          if (page.props.flash?.success) {
    ...
    } else if (action === 'download-payslip') {
      window.location.href = route('hr.payslips.download', entry.payslip.id);
    }
  };
  ...
  return (
    <PageTemplate
      title={payrollRun.title}
      url={`/payroll-runs/${payrollRun.id}`}
      actions={pageActions}
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('Held Employees')}</CardTitle>
            <Users className="h-4 w-4 text-orange-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{heldCount}</div>
            <p className="text-xs text-muted-foreground mt-1">{t('Salaries on hold')}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('Held Amount')}</CardTitle>
            <DollarSign className="h-4 w-4 text-orange-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{window.appSettings?.formatCurrency(heldAmount)}</div>
            <p className="text-xs text-muted-foreground mt-1">{t('Total held salary')}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CrudTable
            columns={[
              ...
              },
              {
                ...
                render: (value: number) => `${window.appSettings.formatDuration(value)}h`
              },
              {
                key: 'overtime_amount',
              ...
              },
              {
                ...
                render: (value: any, row: any) => {
                  const advances = row.employee?.employee_advances || [];
                  const lifetimeTotal = advances.reduce((sum: number, adv: any) => {
                    if (adv.status === 'rejected' || adv.status === 'pending') return sum;
                    return sum + parseFloat(adv.amount || 0);
                  }, 0);
                  return (
                    <span className="font-mono text-blue-600">{window.appSettings?.formatCurrency(lifetimeTotal)}</span>
                  );
                }
              },
              {
                ...
                label: t('Paid Amount'),
                render: (value: any, row: any) => {
                  const allocations = row.advance_allocations || [];
                  const paidThisRun = allocations.reduce((sum: number, a: any) => sum + parseFloat(a.amount || 0), 0);
                  return (
                    <span className="font-mono text-green-600 font-bold">{window.appSettings?.formatCurrency(paidThisRun)}</span>
                  );
                }
              },
              {
                ...
                render: (value: any, row: any) => {
                  const advances = row.employee?.employee_advances || [];
                  const totalPendingAfter = advances.reduce((sum: number, adv: any) => {
                    if (adv.status === 'rejected' || adv.status === 'pending') return sum;
                    return sum + (parseFloat(adv.amount || 0) - parseFloat(adv.paid_amount || 0));
                  }, 0);
                  return (
                    <span className="font-mono text-red-600 font-bold">{window.appSettings?.formatCurrency(totalPendingAfter)}</span>
                  );
                }
              },
              {
            ...
            actions={[
              {
                label: t('Regenerate Payroll'),
                icon: 'RefreshCw',
                action: 'regenerate-employee',
                className: 'text-amber-600',
                condition: () => canRegenerateEmployee,
              },
              {
                ...
                condition: (row: any) => !row.payslip && payrollRun.status === 'completed'
              },
              {
