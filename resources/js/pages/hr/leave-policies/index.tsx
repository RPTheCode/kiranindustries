...
export default function LeavePolicies() {
  ...
  const { auth, leavePolicies, leaveTypes, categories, filters: pageFilters = {} } = usePage().props as any;
      ...
      router.post(route('hr.leave-policies.store'), formData, {
        onSuccess: (page) => {
          ...
          if (pageProps.flash.success) {
            toast.success(t(pageProps.flash.success));
          } else if (pageProps.flash.error) {
            toast.error(t(pageProps.flash.error));
          }
        },
        onError: (errors) => {
          toast.dismiss();
  ...
  const columns = [
    ...
    },
    {
      ...
      label: t('Applicable Categories'),
      render: (value: any) => {
        if (!value || !Array.isArray(value) || value.length === 0) return <span>{t('All Categories')}</span>;
        return (
          <div className="flex flex-wrap gap-1">
            {value.map((catId: any) => {
              const category = categories.find((c: any) => c.id.toString() === catId.toString());
              return (
                <span key={catId} className="inline-flex items-center rounded-md bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">
  ...
  // Prepare leave type options for filter
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              label: t('Applicable Categories'),
              type: 'multi-select',
              placeholder: t('All Categories (If empty)'),
