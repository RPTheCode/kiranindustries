...
export default function EmployeeSalaries() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: number) => (
        <span className="font-mono text-green-500">{window.appSettings?.formatCurrency(value || 0)}</span>
      )
    },
    {
      key: 'minimum_wages',
    ...
    {
      key: 'min_wage_per_day',
      label: t('Min Wage Per Day'),
      render: (value: number) => (
        <span className="font-mono text-blue-500">{window.appSettings?.formatCurrency(value || 0)}</span>
      )
    },
    {
      key: 'components',
      ...
      <CrudFormModal
        ...
        formConfig={{
          fields: [
            ...
            { name: 'per_day_salary', label: t('Per Day Salary'), type: 'number', min: 0, step: 0.01 },
            { name: 'minimum_wages', label: t('Minimum Wages'), type: 'number', min: 0, step: 0.01 },
            { name: 'min_wage_per_day', label: t('Minimum Wage Per Day'), type: 'number', min: 0, step: 0.01 },
            {
              name: 'components',
        ...
        initialData={currentItem ? {
          ...
          components: currentItem.components 
            ? (Array.isArray(currentItem.components) 
                ? currentItem.components.map((id: any) => id.toString())
                : Object.keys(currentItem.components).map((id: any) => id.toString()))
            : []
        } : null}
        title={
          formMode === 'create'
