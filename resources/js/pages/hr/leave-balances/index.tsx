...
export default function LeaveBalances() {
  ...
  const fetchCarryForward = (employeeId: any, leaveTypeId: any, year: any, setFormData: any) => {
    if (employeeId && leaveTypeId && year) {
      fetch(route('hr.leave-balances.suggest-carry-forward', {
        ...
        .then(data => {
          if (data.amount !== undefined) {
            setFormData((prev: any) => ({ ...prev, carried_forward: data.amount }));
            // toast.info(t('Suggested carry forward: ') + data.amount);
          }
        })
        .catch(err => console.error('Failed to fetch carry forward suggestion', err));
    }
  };
          ...
          fields: [
            {
              ...
              onChange: (value, formData, setFormData) => {
                if (formMode === 'create') {
                  fetchCarryForward(formData.employee_id, value, formData.year, setFormData);
                }
              }
            },
            {
              ...
              onChange: (value, formData, setFormData) => {
                if (formMode === 'create') {
                  fetchCarryForward(value, formData.leave_type_id, formData.year, setFormData);
                }
              }
            },
            {
              ...
              onChange: (value, formData, setFormData) => {
                if (formMode === 'create') {
                  fetchCarryForward(formData.employee_id, formData.leave_type_id, value, setFormData);
                }
              }
            },
