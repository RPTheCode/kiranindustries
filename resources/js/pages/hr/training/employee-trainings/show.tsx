...
export default function EmployeeTrainingShow() {
  ...
  const handleFormSubmit = (formData: any) => {
    ...
    Object.keys(formData).forEach(key => {
      ...
      } else if (key === 'is_passed') {
        // Handle boolean field properly - convert to 0 or 1 for Laravel
        if (formData[key]) {
          data.append(key, '1');
        } else {
          data.append(key, '0');
  ...
  const handleAssessmentSubmit = (formData: any) => {
    ...
    // Convert boolean field for checkbox
    const data = {
      ...formData,
      is_passed: formData.is_passed ? 1 : 0
    };
  ...
  return (
    <PageTemplate
      title={`${employeeTraining.employee?.name || ''} - ${employeeTraining.training_program?.name || ''}`}
      url={`/training/employee-trainings/${employeeTraining.id}`}
      actions={pageActions}
