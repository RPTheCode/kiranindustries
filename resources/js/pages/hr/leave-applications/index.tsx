  ...
  const columns = [
    ...(!self_service_only ? [{
      ...
      render: (value: any, row: any) => {
        const empId = row.employee?.employee?.employee_id;
        const branchName = row.employee?.employee?.branch?.name;
        return (
          <div className="flex flex-col">
            <span className="font-medium text-gray-900">
              {row.employee?.name || '-'}
              {branchName && <span className="ml-1 text-xs text-muted-foreground">({branchName})</span>}
            </span>
            {empId && <span className="text-xs text-gray-500">{empId}</span>}
          </div>
        );
      }
    }] : []),
