...
export default function Designations() {
  ...
  const { auth, designations, departments, branches = [], stats = {}, activeBranchId, filters: pageFilters = {} } = usePage().props as any;
  ...
  const departmentFilterFromUrl = pageFilters.department ? String(pageFilters.department) : 'all';

  // State
  const [searchTerm, setSearchTerm] = useState(pageFilters.search || '');
  const [selectedDepartment, setSelectedDepartment] = useState(departmentFilterFromUrl);
  ...
  useEffect(() => {
    ...
    setSelectedDepartment(departmentFilterFromUrl);
    if (pageFilters.status) {
      setSelectedStatus(pageFilters.status);
    }
  }, [pageFilters.branch_id, pageFilters.department, pageFilters.status]);
  ...
  const filteredDepartmentName =
    selectedDepartment !== 'all'
      ? departments.find((d: any) => String(d.id) === selectedDepartment)?.name
      : null;
  ...
  const handleSearchClear = () => {
    setSearchTerm('');
    router.get(route('hr.designations.index'), listQueryParams({ search: undefined }), {
      preserveState: true,
      preserveScroll: true,
    });
  };
  ...
  const applyFilters = () => {
    router.get(route('hr.designations.index'), listQueryParams(), { preserveState: true, preserveScroll: true });
  };
        ...
        if (!item.can_delete) {
          toast.error(
            item.delete_block_reason ||
              t('This designation is assigned to employees in this branch and cannot be deleted.')
          );
          return;
        }
  ...
  const breadcrumbs = [
    ...
    { title: t('Designation Management'), href: route('hr.designations.index') },
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        if (!dept?.name) return <span className="text-slate-400 text-sm">—</span>;
        if (!hasPermission(permissions, 'view-employees') || !branchId) {
          return <span className="text-sm text-slate-700 truncate max-w-[9rem] block" title={dept.name}>{dept.name}</span>;
        }
        return (
          <button
            type="button"
            ...
            className="text-sm font-medium theme-color hover:opacity-80 hover:underline truncate max-w-[9rem] block text-left border-none bg-transparent p-0 cursor-pointer"
          >
            {dept.name}
          </button>
        );
    ...
    },
    {
      ...
      render: (_: number, record: any) => {
        ...
        return (
          <button
            type="button"
            title={t('View employees on this designation')}
            onClick={() =>
              router.get(route('hr.employees.index'), {
                branch: branchId,
      ...
      : []),
    {
      ...
      render: (_: any, record: any) => {
        ...
        const canToggle =
          hasPermission(permissions, 'toggle-status-designations') || hasPermission(permissions, 'edit-designations');
  ...
  return (
    ...
    >
      {filteredBranchName && isViewingOtherBranch && (
        ...
        >
          <span>
            {t('Showing designations for')}: <strong>{filteredBranchName}</strong>
          </span>
          <button
            type="button"
            onClick={() => {
              ...
              router.get(route('hr.designations.index'), listQueryParams({ branch_id: activeId }), {
                preserveState: true,
                preserveScroll: true,
              });
            }}
      ...
      {filteredDepartmentName && selectedDepartment !== 'all' && (
        <div
          className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700"
        >
          <span>
            {t('Filtered by department')}: <strong>{filteredDepartmentName}</strong>
          </span>
          <button
            type="button"
            onClick={() => {
              setSelectedDepartment('all');
              router.get(route('hr.designations.index'), listQueryParams({ department: 'all' }), {
                preserveState: true,
                preserveScroll: true,
              });
            }}
            className="text-xs font-semibold text-slate-600 underline border-none bg-transparent cursor-pointer hover:opacity-80"
          >
            {t('Show all departments')}
          </button>
        </div>
      )}
