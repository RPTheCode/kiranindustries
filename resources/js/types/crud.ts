...
export interface FormField {
  name: string;
  label: string;
  type: 'text' | 'email' | 'password' | 'select' | 'combobox' | 'textarea' | 'radio' | 'checkbox' | 'switch' | 'file' | 'date' | 'time' | 'number' | 'multi-select' | 'media-picker' | 'custom' | 'dependent-dropdown' | 'color';
  placeholder?: string;
  required?: boolean;
  multiple?: boolean; // For media-picker and multi-select fields
  options?: FilterOption[] | ((data: any) => FilterOption[]);
  relation?: {
    endpoint: string;
    valueField: string;
    labelField: string;
  ...
  onChange?: (value: any, formData: any, setFormData: (setter: (prev: any) => any) => void) => void;
}
