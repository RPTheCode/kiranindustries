...
class AnnouncementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Branches dropdown removed or simplified (if needed we can fetch just active branch, but usually hidden)
        $branches = [];

        // Filter departments - show all departments of the company (unfiltered by branch)
        ...
        return Inertia::render('hr/announcements/index', [
            ...
            'filters' => $request->all(['search', 'category', 'department_id', 'status', 'priority', 'featured', 'date_from', 'date_to', 'sort_field', 'sort_direction', 'per_page']),
        ]);
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'department_ids' => 'nullable|required_if:is_company_wide,false',
            // branch_ids removed, auto-handled
        ]);
        ...
        // Check if departments belong to current company (and optional check for active branch)
        if (
            !$request->is_company_wide &&
            empty($request->department_ids)
        ) {
            // If not company wide, and no departments, we assume "Whole Active Branch" target?
            // or we require at least one target.
            // Since we will auto-attach active branch, this is fine.
        }
        ...
        if (!empty($request->department_ids)) {
            ...
            $departmentIds = array_filter($departmentIds); // Remove empty values
            
            if (!empty($departmentIds)) {
                $validDepartmentsCount = Department::whereIn('created_by', getCompanyAndUsersId())
        ...
        if (!$request->is_company_wide) {
            ...
            $activeBranchId = session('active_branch_id');
            if ($activeBranchId) {
                $announcement->branches()->attach([$activeBranchId]);
            }
        }
    ...
    public function update(Request $request, Announcement $announcement)
    {
        ...
        if ($request->is_company_wide) {
            ...
            $activeBranchId = session('active_branch_id');
            if ($activeBranchId) {
                $announcement->branches()->sync([$activeBranchId]);
            } else {
                // Do not detach if no active branch? Or detach all? 
                // If editing without active branch, we shouldn't touch branches?
                // But we are enforcing active branch context.
                // If active branch is present, sync it.
            }
        }
