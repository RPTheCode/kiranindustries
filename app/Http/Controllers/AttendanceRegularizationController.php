...
class AttendanceRegularizationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $attendanceRecord = AttendanceRecord::withoutGlobalScopes()->find($validated['attendance_record_id']);
        if (!$attendanceRecord) {
            return redirect()->back()->with('error', __('Attendance record not found.'));
        }

        // Verify employee exists (bypass branch scope)
        $employee = \App\Models\Employee::withoutGlobalScopes()->where('user_id', $validated['employee_id'])->first();
        if (!$employee) {
            return redirect()->back()->with('error', __('Selected employee profile not found.'));
        }

        // Regularization should be associated with the branch where the attendance occurred
        $validated['branch_id'] = $attendanceRecord->branch_id;
    ...
    public function update(Request $request, $regularizationId)
    {
        ...
        if ($regularization) {
            try {
                ...
                // Verify employee profile exists (bypass branch scope)
                $employee = \App\Models\Employee::withoutGlobalScopes()->where('user_id', $validated['employee_id'])->first();
                if (!$employee) {
                    return redirect()->back()->with('error', __('Selected employee profile not found.'));
                }

                // Get attendance record to derive branch (bypass branch scope)
                $attendanceRecord = AttendanceRecord::withoutGlobalScopes()->find($validated['attendance_record_id']);
                if (!$attendanceRecord) {
                    return redirect()->back()->with('error', __('Attendance record not found.'));
                }
    ...
    public function getEmployeeAttendance($employeeId)
    {
        try {
            ...
            // Get attendance records for the current month in the active branch
            $attendanceRecords = AttendanceRecord::withoutGlobalScopes()
            ...
            $datesForDropdown = $attendanceRecords->map(function ($record) {
                $date = $record->date instanceof \Carbon\Carbon ? $record->date : \Carbon\Carbon::parse($record->date);
                return [
