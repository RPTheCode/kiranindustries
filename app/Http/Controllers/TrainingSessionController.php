...
class TrainingSessionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Get employees for trainer dropdown - Show all employees from all branches
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->trainer_ids)) {
            ...
            if (count($validTrainers) !== count($trainerIds)) {
                return redirect()->back()->with('error', __('Invalid trainer selection'));
            }
        }
    ...
    public function show(TrainingSession $trainingSession)
    {
        ...
        // Get all employees assigned to this training program
        $assignedEmployees = EmployeeTraining::where('training_program_id', $trainingSession->training_program_id)
        ...
        // Merge attendance data - include all assigned employees, with their attendance status if it exists
        $attendanceData = $assignedEmployees->map(function ($employeeTraining) use ($existingAttendance) {
            $attendance = $existingAttendance->get($employeeTraining->employee_id);

            return [
                'employee_id' => $employeeTraining->employee_id,
                'name' => $employeeTraining->employee->name,
                'employee_id_display' => $employeeTraining->employee->employee->employee_id ?? '-',
