...
class User extends BaseAuthenticatable implements MustVerifyEmail
{
    ...
    /**
     * Get the branches associated with the user/company.
     */
    public function branches()
    {
        if ($this->type === 'company' || $this->type === 'admin') {
            $localKey = $this->creatorId() == $this->id ? 'id' : 'created_by';
            return $this->hasMany(Branch::class, 'created_by', $localKey);
        }
        return $this->assignedBranches();
    }

    /**
     * Get the assigned branches (explicitly via pivot table).
     */
    ...
    /**
     * Get the work history associated with the user
     */
    public function employeeWorkHistories()
    {
        return $this->hasMany(EmployeeWorkHistory::class, 'employee_id');
    }
    ...
    /**
     * Get active employee advances (where amount > paid_amount).
     */
    public function activeAdvances()
    {
        return $this->hasMany(EmployeeAdvance::class, 'employee_id')
            ->where('status', 'paid') // Only approved/paid-out advances
    ...
    private function getManagerPermissions(): array
    {
        return [
            ...
            'manage-salary-payroll-employee-salary',
            'manage-any-salary-payroll-employee-salary',
            'manage-own-salary-payroll-employee-salary',
            'view-salary-payroll-employee-salary',
            'create-salary-payroll-employee-salary',
            'edit-salary-payroll-employee-salary',
            'delete-salary-payroll-employee-salary',
            'manage-salary-payroll-increment',
            'manage-any-salary-payroll-increment',
            'view-salary-payroll-increment',
            'create-salary-payroll-increment',
            'edit-salary-payroll-increment',
            ...
            'manage-earning-deduction-entry',
            'manage-any-earning-deduction-entry',
            'manage-own-earning-deduction-entry',
            'view-earning-deduction-entry',
            'create-earning-deduction-entry',
            'edit-earning-deduction-entry',
            'delete-earning-deduction-entry',
