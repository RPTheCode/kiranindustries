...
trait AutoApplyPermissionCheck
{
    ...
    {   
        ...
        try {
            if ($user->hasPermissionTo("manage-any-{$moduleSlug}")) {
        ...
        // 3. Company role check: show their records and their employees' records
        ...
        try {
            // 4. Specific permissions: manage-own
            if ($user->hasPermissionTo("manage-own-{$moduleSlug}")) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'created_by')) {
            ...
            try {
                if ($user->hasPermissionTo("access-{$moduleSlug}-module")) {
            ...
            } catch (PermissionDoesNotExist $e2) {
                // Both permissions missing, continue
            }
        }

        // 5. Employee role check
        if ($user->hasRole(['employee'])) {
            return $this->applyEmployeeRoleFiltering($query, $user, $permission = null, $moduleSlug);
        }

        // 6. Default manage permission
        try {
            if ($user->hasPermissionTo("manage-{$moduleSlug}")) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'created_by')) {
                    return $query->whereIn('created_by', getCompanyAndUsersId());
                }
            }
